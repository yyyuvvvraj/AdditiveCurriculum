...
function KanbanCard({ po, onMove }: { po: PurchaseOrder, onMove: (id: string, next: PurchaseOrder['status']) => void }) {
  ...
  return (
    <div className={`group bg-white p-4 rounded-xl border shadow-sm hover:shadow-md transition-all mb-3 relative overflow-hidden ${isUrgent ? 'border-l-4 border-l-red-500' : 'border-l-4 border-l-slate-300'}`}>
      
      <div className="flex justify-between items-start mb-2">
        <span className="text-xs font-bold text-slate-400 tracking-mono">{po.id}</span>
        {isUrgent && (
            <span className="flex items-center gap-1 text-[10px] bg-red-50 text-red-600 px-2 py-0.5 rounded-full font-bold uppercase border border-red-100">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span> Urgent
            </span>
        )}
      </div>
      
      <h4 className="font-bold text-slate-800 text-sm leading-tight">{po.partName}</h4>
      <div className="text-xs text-slate-500 mt-1 flex justify-between items-center">
        <span className="font-medium text-slate-600">{po.vendor}</span>
        <span className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 font-mono">Qty: {po.qty}</span>
      </div>
      
      <div className="mt-3 pt-3 border-t border-slate-100 flex justify-between items-center">
        <span className="font-mono text-xs font-bold text-slate-700">₹{po.cost.toLocaleString()}</span>
        
        {po.status !== 'Received' && (
            <button 
            onClick={() => onMove(po.id, nextStatus[po.status])}
            className="opacity-0 group-hover:opacity-100 transition-opacity px-2 py-1 text-[10px] font-bold uppercase bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded flex items-center gap-1"
            >
...
function WorkOrderRow({ wo, onResolve }: { wo: WorkOrder, onResolve: (id: string, machineId?: string) => void }) {
    return (
        <div className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-xl shadow-sm hover:shadow-md transition-all mb-3">
            <div className="flex items-start gap-4">
                <div className={`p-2 rounded-lg ${wo.priority === 'High' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
                </div>
                <div>
                    <div className="font-bold text-slate-800">{wo.machineName}</div>
                    <div className="text-xs text-slate-500">Issue: <span className="font-medium text-slate-700">{wo.issue}</span></div>
                    <div className="text-[10px] text-slate-400 mt-1">{new Date(wo.timestamp).toLocaleString()}</div>
                </div>
            </div>
            <div className="flex items-center gap-3">
                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${wo.status === 'Pending' ? 'bg-amber-100 text-amber-700' : 'bg-indigo-100 text-indigo-700'}`}>{wo.status}</span>
                {wo.status !== 'Resolved' && (
                    <button onClick={() => onResolve(wo.id, wo.machineId)} className="px-3 py-1.5 text-xs font-medium bg-emerald-500 text-white rounded-lg shadow hover:bg-emerald-600 transition-colors">Complete</button>
                )}
            </div>
        </div>
    );
    ...
    <div className="min-h-screen bg-slate-50/50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between mb-8 gap-4">
            <div>
                <h1 className="text-3xl font-extrabold text-slate-900">Plant Command Center</h1>
            ...
            <div className="flex bg-white p-1 rounded-lg border shadow-sm self-start">
                <button onClick={() => setActiveTab('overview')} className={`px-4 py-2 text-sm font-bold rounded-md transition-all ${activeTab === 'overview' ? 'bg-slate-800 text-white shadow' : 'text-slate-500 hover:bg-slate-50'}`}>
                    Operations
                </button>
                <button onClick={() => setActiveTab('procurement')} className={`px-4 py-2 text-sm font-bold rounded-md transition-all ${activeTab === 'procurement' ? 'bg-indigo-600 text-white shadow' : 'text-slate-500 hover:bg-slate-50'}`}>
                    Procurement Board
                </button>
            </div>
        </div>
            ...
            <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white p-5 rounded-2xl border shadow-sm flex items-center justify-between">
                        <div><div className="text-sm text-slate-500 font-medium">Critical Alerts</div><div className="text-3xl font-extrabold text-red-600 mt-1">{activeAlerts}</div></div>
                        <div className="p-3 bg-red-50 text-red-500 rounded-xl"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg></div>
                    </div>
                    <div className="bg-white p-5 rounded-2xl border shadow-sm flex items-center justify-between">
                        <div><div className="text-sm text-slate-500 font-medium">Pending Work Orders</div><div className="text-3xl font-extrabold text-indigo-600 mt-1">{workOrders.filter(w => w.status !== 'Resolved').length}</div></div>
                        <div className="p-3 bg-indigo-50 text-indigo-500 rounded-xl"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg></div>
                    </div>
                    <div className="bg-white p-5 rounded-2xl border shadow-sm flex items-center justify-between">
                        <div><div className="text-sm text-slate-500 font-medium">Low Stock Items</div><div className="text-3xl font-extrabold text-amber-600 mt-1">{lowStockItems}</div></div>
                        <div className="p-3 bg-amber-50 text-amber-500 rounded-xl"><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg></div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-200">
                            <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><span className="w-2 h-2 bg-indigo-500 rounded-full"></span> Live Work Order Queue</h2>
                            <div className="space-y-2">
                                {workOrders.filter(w => w.status !== 'Resolved').length === 0 ? <div className="text-center py-12 text-slate-400">No pending work orders.</div> : workOrders.filter(w => w.status !== 'Resolved').map((wo) => (<WorkOrderRow key={wo.id} wo={wo} onResolve={handleResolveWO} />))}
                            </div>
                        </div>
                    </div>
                    <div className="space-y-6">
                        <div className="bg-white p-5 rounded-2xl border shadow-lg">
                            <h3 className="font-bold text-slate-800 mb-4">Fleet Health Matrix</h3>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="md:col-span-2 flex justify-end items-center">
                        <button onClick={() => setIsPOModalOpen(true)} className="px-6 py-3 bg-indigo-600 text-white text-sm font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition-all hover:scale-[1.02] flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 h-full min-h-[600px] overflow-x-auto pb-4">
                    {[
                        { id: 'Request', label: 'Draft Request', color: 'bg-slate-100 border-slate-200' },
                        { id: 'Ordered', label: 'Placed with Vendor', color: 'bg-amber-50 border-amber-200' },
                        { id: 'In Transit', label: 'Shipped / In Transit', color: 'bg-indigo-50 border-indigo-200' },
                        { id: 'Received', label: 'Delivered & Stocked', color: 'bg-emerald-50 border-emerald-200' }
                    ].map((col) => (
                        <div key={col.id} className={`flex flex-col h-full rounded-2xl border ${col.color} p-4`}>
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <span className="bg-white/50 text-slate-600 text-xs font-bold px-2 py-1 rounded-md border border-black/5 shadow-sm">
                            ...
                            <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar">
                                ...
                                {purchaseOrders.filter(p => p.status === col.id).length === 0 && (
                                    <div className="h-32 border-2 border-dashed border-black/5 rounded-xl flex items-center justify-center text-xs text-slate-400 italic">
                                        No items
                                    </div>
                                )}
                            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in zoom-in-95 duration-200">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                  <div className="flex justify-between items-center mb-6 border-b pb-4">
                      ...
                      <button onClick={() => setIsPOModalOpen(false)} className="text-slate-400 hover:text-slate-600">✕</button>
                  </div>
                  
                  <div className="space-y-5">
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Select Part</label>
                          <select className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all" onChange={(e) => setNewPO({...newPO, partId: e.target.value})}>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <div>
                              ...
                              <input type="number" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-indigo-500 outline-none" value={newPO.qty} onChange={(e) => setNewPO({...newPO, qty: parseInt(e.target.value)})} />
                          </div>
                          <div>
                              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Urgency</label>
                              <select className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none" value={newPO.urgency} onChange={(e) => setNewPO({...newPO, urgency: e.target.value as any})}>
                  ...
                  <div className="flex justify-end gap-3 mt-8">
                      <button onClick={() => setIsPOModalOpen(false)} className="px-5 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">Cancel</button>
                      <button onClick={handleCreatePO} className="px-5 py-2.5 text-sm font-bold bg-indigo-600 text-white rounded-xl shadow-lg hover:bg-indigo-700 transition-all active:scale-95">Submit Request</button>
                  </div>
              </div>
          </div>
      )}
