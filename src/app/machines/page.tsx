  ...
  return (
    <div className={`relative flex items-center justify-center shrink-0 ${size === "lg" ? "w-20 h-20" : "w-14 h-14"}`}>
      <svg className={`transform -rotate-90 ${size === "lg" ? "w-20 h-20" : "w-14 h-14"}`}>
        {/* Background Ring */}
        <circle cx={width/2} cy={width/2} r={radius} stroke="currentColor" strokeWidth={size==="lg"?6:4} fill="transparent" className="text-slate-200 opacity-50" />
        {/* Value Ring */}
...
function MetricBar({ label, value, max, unit }: { label: string, value: number, max: number, unit: string }) {
    ...
    const barColor = percent > 85 ? 'bg-red-400' : percent > 60 ? 'bg-amber-400' : 'bg-emerald-400';
    return (
...
function LiveSparkline({ baseValue, variance, color }: { baseValue: number, variance: number, color: string }) {
  ...
  return (
    <div className="h-12 w-full mt-2 opacity-60 overflow-hidden relative">
      ...
      <div className={`absolute inset-0 bg-gradient-to-t from-white/0 via-white/0 to-transparent`}></div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className={`px-6 py-4 border-b flex justify-between items-center ${styles.cardBg} ${styles.border} border-b`}>
            <div>
                <h2 className={`text-xl font-bold ${styles.text}`}>{machine.name}</h2>
                <div className="text-sm text-slate-600 flex gap-2">ID: {machine.id} • Status: <span className="font-bold">{machine.status}</span></div>
            </div>
            <button onClick={onClose} className="p-2 bg-white/50 hover:bg-white rounded-full text-slate-500 transition-colors">✕</button>
        </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 overflow-y-auto">
            <div className="md:col-span-1 space-y-6">
                <div className="flex flex-col items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <span className="text-sm font-semibold text-slate-500 mb-2">Health Index</span>
            ...
            <div className="md:col-span-2 space-y-5">
                <div className={`p-4 rounded-xl border ${styles.cardBg} ${styles.border}`}>
                    <h3 className="font-bold text-slate-800 mb-1">AI Diagnostics</h3>
                    <p className={`font-medium text-sm mb-2 ${styles.text}`}>{machine.prediction}</p>
                    <p className="text-xs text-slate-500"><span className="font-semibold">Root Cause:</span> {machine.rootCause}</p>
                </div>
                <div className="bg-white p-4 rounded-xl border shadow-sm">
                    <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wide mb-3">Recommended Actions</h4>
                    <div className="space-y-2">
                        {machine.suggestedActions?.map((a, i) => (
                            <label key={i} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded cursor-pointer border border-transparent hover:border-slate-100">
        ...
        <div className="bg-slate-50 px-6 py-4 border-t flex justify-end gap-3 mt-auto">
            <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-800">Cancel</button>
            {machine.status === 'MAINTENANCE' ? (
                <div className="px-4 py-2 text-sm font-bold text-indigo-600 bg-indigo-50 rounded-lg border border-indigo-100 flex items-center gap-2">
                    <span>✓ Maintenance Active</span>
                </div>
            ) : (
                <button onClick={handleCreateWorkOrder} className="px-4 py-2 text-sm font-bold bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700">Create Work Order</button>
            )}
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-slate-50/50">
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row justify-between mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-800">Machine Status</h1>
            <div className="text-sm text-slate-500 mt-1">Showing {filteredMachines.length} units • Live Telemetry</div>
          </div>
          <div className="flex flex-wrap items-center gap-3">
             <button onClick={() => setSortOrder(p => p === 'asc' ? 'desc' : 'asc')} className="px-4 py-2 bg-white border rounded-lg text-sm font-medium text-slate-700 shadow-sm">
                Sort: {sortOrder === 'asc' ? 'Worst First' : 'Best First'}
             </button>
             <div className="flex bg-white p-1 rounded-lg border shadow-sm">
                {['ALL', 'NORMAL', 'WARNING', 'CRITICAL', 'MAINTENANCE'].map(status => (
                    <button key={status} onClick={() => setFilterStatus(status)} 
                        className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase ${filterStatus === status ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>
        ...
        {loading ? <div className="flex justify-center h-64 text-slate-400">Loading live telemetry...</div> : (
                ...
                >
                    <div className="flex justify-between items-start mb-4 gap-3">
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="font-bold text-slate-800 text-base leading-snug break-words">{m.name}</div>
                    ...
                    <div className="mb-4 px-3 py-2 rounded-lg text-xs border bg-white/60 border-black/5">
                        <span className={`font-bold opacity-75 mr-1 ${styles.text}`}>STATUS:</span>
                        <span className="text-slate-700">{m.prediction}</span>
                    </div>
